...
def load_cost2100_outdoor_dataset(data_path=None, mode="train", normalization=True, scenario="out"):
    ...
    dir_train = os.path.join(datapath_cost2100, f"DATA_Htrain{scenario}.mat")
    dir_val = os.path.join(datapath_cost2100, f"DATA_Hval{scenario}.mat")
    dir_test = os.path.join(datapath_cost2100, f"DATA_Htest{scenario}.mat")
    dir_raw = os.path.join(datapath_cost2100, f"DATA_HtestF{scenario}_all.mat")
    ...
    x_train_uplink = data['x_train_uplink']
    x_val_uplink = data['x_val_uplink']
    x_test_uplink = data['x_test_uplink']
    ...
    # overwrite with cost2100 data
    num_train_samps = x_train_downlink.shape[0]
    x_train_downlink = data_train[:num_train_samps,:,:,:,0] # train input
    
    num_test_samps = x_test_downlink.shape[0]
    x_test_downlink = data_test[:num_test_samps,:,:,:,0] # test input  
