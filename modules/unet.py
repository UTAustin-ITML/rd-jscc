...
class Downsample(tf.keras.layers.Layer):
    def __init__(self, dim_in, dim_out=None):
        ...
        if dim_out is None:
            dim_out = dim_in
        # Use strided convolution with 'same' padding for efficiency
...
class Block(tf.keras.layers.Layer):
    def __init__(self, dim, dim_out, large_filter=False):
        ...
        # Use 'same' padding to avoid explicit ZeroPadding2D layer for standard case
        if large_filter:
            self.padding = tf.keras.layers.ZeroPadding2D(padding=3, data_format='channels_first')
            self.conv = tf.keras.layers.Conv2D(dim_out, kernel_size=7, padding='valid',
                                              data_format='channels_first', use_bias=False)
        else:
            self.padding = None
            self.conv = tf.keras.layers.Conv2D(dim_out, kernel_size=3, padding='same',
                                              data_format='channels_first', use_bias=False)

        self.layer_norm = tf.keras.layers.LayerNormalization(axis=[1, 2, 3], epsilon=1e-5)
...
class ResnetBlock(tf.keras.layers.Layer):
    def __init__(self, dim, dim_out, time_emb_dim=None, large_filter=False):
        ...
        self.mlp = (tf.keras.Sequential([
            ...
            tf.keras.layers.Dense(dim_out, use_bias=False)
        ]) if exists(time_emb_dim) else None)

        self.block1 = Block(dim, dim_out, large_filter)
        self.block2 = Block(dim_out, dim_out)
        # Use 1x1 conv without bias for residual projection
        self.res_conv = (tf.keras.layers.Conv2D(dim_out, 1, data_format='channels_first',
                                                use_bias=False) if dim != dim_out else tf.keras.layers.Identity())
    ...
    def call(self, x, time_emb=None):
        ...
        if exists(time_emb):
            y = self.mlp(time_emb)
            # Efficiently add time embedding
            h = h + tf.reshape(y, [-1, tf.shape(y)[-1], 1, 1])
...
class LinearAttention(tf.keras.layers.Layer):
    def __init__(self, dim, heads=1, dim_head=None):
        ...
        # Fused QKV computation for efficiency
        self.to_qkv = tf.keras.layers.Conv2D(hidden_dim * 3, 1, use_bias=False,
                                             data_format="channels_first")
        self.to_out = tf.keras.layers.Conv2D(dim, 1, data_format="channels_first", use_bias=False)
    ...
    def call(self, x):
        ...
        qkv = self.to_qkv(x)
        qkv = tf.reshape(qkv, [b, 3, self.heads, self.dim_head, h * w])
        ...
        # Linear attention: compute context then apply to query
        context = tf.einsum("bhdn,bhen->bhde", k, v)
        out = tf.einsum("bhde,bhdn->bhen", context, q)

        # Reshape back to spatial dimensions
        out = tf.reshape(out, [b, self.heads * self.dim_head, h, w])
...
class Unet(Model):
    def __init__(
        ...
        # Dynamically configure context dimensions
        context_dims = [2, 16, 32]

        # Pre-build pooling layers for context downsampling
        self.context_poolers = []
        for i in range(len(context_dims)):
            if i == 0:
                # No downsampling needed for first context
                self.context_poolers.append(None)
            elif i == 1:
                # 2x downsampling
                self.context_poolers.append(tf.keras.layers.AveragePooling2D(
                    pool_size=2, strides=2, data_format='channels_first'))
            elif i == 2:
                # 4x downsampling
                self.context_poolers.append(tf.keras.layers.AveragePooling2D(
                    pool_size=4, strides=4, data_format='channels_first'))
            else:
                # Generic power-of-2 downsampling
                factor = 2 ** i
                self.context_poolers.append(tf.keras.layers.AveragePooling2D(
                    pool_size=factor, strides=factor, data_format='channels_first'))

        in_out = list(zip(dims[:-1], dims[1:]))
        self.embd_type = embd_type
        self.num_context_levels = len(context_dims)
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append([
                ResnetBlock(
                    dim_in + context_dims[ind] if (not is_last) and (ind < (len(context_dims) - 1)) else dim_in,
                    dim_out,
                    time_dim,
    ...
    def _preprocess_context(self, context, idx):
        """Efficiently preprocess context at given index with appropriate downsampling."""
        if idx >= len(context) or idx >= self.num_context_levels:
            return None

        ctx = context[idx]
        if self.context_poolers[idx] is not None:
            ctx = self.context_poolers[idx](ctx)
        return ctx

    def encode(self, x, t, context):
        h = []
        # context=[context_output_1, context_output_2, ...]
        for idx, (backbone, backbone2, attn, downsample) in enumerate(self.downs):
            # Efficiently process and concatenate context
            ctx = self._preprocess_context(context, idx)
            if ctx is not None:
